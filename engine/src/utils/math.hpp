#pragma once

#include <cmath>
#include <limits>
#include <numeric>
#include <numbers>
#include <ranges>

#include "utils/types.hpp"

namespace ii {

namespace MathConfig {
    constexpr f64 epsilon = 1e-10f;
    constexpr f64 pi = std::numbers::pi;
    constexpr f64 inv_pi = std::numbers::inv_pi;
    constexpr f64 inf = std::numeric_limits<f64>::infinity();
    constexpr f64 G = 6.67430e-11; // gravitational constant
    constexpr f64 c = 299792458;   // speed of light in vacuum
    constexpr f64 au = 1.495978707e11; // astronomical unit in meters
    
    inline bool is_equal(f64 a, f64 b, f64 tol = epsilon) {
        return std::abs(a - b) <= tol;
    }

    inline bool is_zero(f64 a, f64 tol = epsilon) {
        return is_equal(a, 0.0, tol);
    }

    inline f64 safe_div(f64 n, f64 d, f64 default_value = 0.0) {
        return is_zero(d) ? default_value : n / d;
    }

    inline f64 eps_div(f64 n, f64 d, f64 tol = epsilon) {
        return is_zero(d, tol) ? (n >= 0.0 ? inf : -inf) : n / d;
    }

    inline f64 clamp(f64 value, f64 min, f64 max) {
        return std::max(min, std::min(max, value));
    }

    inline f64 clamp_angle(f64 angle) {
        angle = std::fmod(angle, 2.0 * pi);
        if (angle < 0.0) {
            angle += 2.0 * pi;
        }
        return angle;
    }

    inline f64 deg_to_rad(f64 degrees) {
        return degrees * (pi / 180.0);
    }

    inline f64 rad_to_deg(f64 radians) {
        return radians * 180.0 * inv_pi;
    }

    inline f64 normalize_angle(f64 angle) {
        angle = std::fmod(angle + pi, 2.0 * pi);
        if (angle < 0.0) {
            angle += 2.0 * pi;
        }
        return angle - pi;
    }

    inline f64 round(f64 value) {
        return std::round(value);
    }

    template <typename State>
    class Integrator {
    public:
        using StepFunction = std::function<State(const State&, f64)>;

        enum class Method { Euler, RK4, VERLET };

        inline explicit Integrator(const Method method = Method::RK4) : method_(method) {}

        inline State integrate(
            const State& start_state, f64 t0, f64 dt, const StepFunction& f
        ) {
            switch (method_) {
            case Method::Euler:
                return euler_step(start_state, t0, dt, f);
            case Method::RK4:
                return rk4_step(start_state, t0, dt, f);
            default:
                throw std::invalid_argument("Unsupported integration method");
            }
        }

    private:
        const Method method_;

        inline State euler_step(const State& state, f64 t, f64 dt, const StepFunction& f) {
            State deriv = f(state, t);
            return state + deriv * dt;
        }

        inline State rk4_step(const State& state, f64 t, f64 dt, const StepFunction& f) {
            State k1 = f(state, t);
            State k2 = f(state + k1 * (dt / 2.0), t + dt / 2.0);
            State k3 = f(state + k2 * (dt / 2.0), t + dt / 2.0);
            State k4 = f(state + k3 * dt, t + dt);
            return state + (k1 + 2.0 * k2 + 2.0 * k3 + k4) * (dt / 6.0);
        }
    };
};

}